...
DWORD intelvirtdisplay_monitor_count = 1;
...
IDDCX_MONITOR g_IntelVirtDisplayCxMonitorObject[MAX_SCAN_OUT] = {0};
IDARG_IN_QUERY_HWCURSOR g_inputargs[MAX_SCAN_OUT] = {0};
Microsoft::WRL::Wrappers::Event g_intelvirtdisplay_cursor_os_event[MAX_SCAN_OUT];
...
EVT_WDF_DRIVER_DEVICE_ADD IntelVirtDisplayUMDDeviceAdd;
EVT_WDF_DEVICE_D0_ENTRY IntelVirtDisplayUMDDeviceD0Entry;
EVT_WDF_DEVICE_D0_EXIT IntelVirtDisplayUMDDeviceD0Exit;
EVT_WDF_DRIVER_UNLOAD IntelVirtDisplayUMDUnload;

EVT_IDD_CX_ADAPTER_INIT_FINISHED IntelVirtDisplayUMDAdapterInitFinished;
EVT_IDD_CX_ADAPTER_COMMIT_MODES IntelVirtDisplayUMDAdapterCommitModes;

EVT_IDD_CX_PARSE_MONITOR_DESCRIPTION IntelVirtDisplayUMDParseMonitorDescription;
EVT_IDD_CX_MONITOR_GET_DEFAULT_DESCRIPTION_MODES IntelVirtDisplayUMDMonitorGetDefaultModes;
EVT_IDD_CX_MONITOR_QUERY_TARGET_MODES IntelVirtDisplayUMDMonitorQueryModes;

EVT_IDD_CX_MONITOR_ASSIGN_SWAPCHAIN IntelVirtDisplayUMDMonitorAssignSwapChain;
EVT_IDD_CX_MONITOR_UNASSIGN_SWAPCHAIN IntelVirtDisplayUMDMonitorUnassignSwapChain;
...
_Use_decl_annotations_ extern "C" NTSTATUS DriverEntry(PDRIVER_OBJECT pDriverObject, PUNICODE_STRING pRegistryPath)
{
	...
	WDF_DRIVER_CONFIG_INIT(&Config, IntelVirtDisplayUMDDeviceAdd);
	Config.EvtDriverUnload = IntelVirtDisplayUMDUnload;
 ...
 *
 ******************************************************************************/
_Use_decl_annotations_ VOID IntelVirtDisplayUMDUnload(WDFDRIVER Driver)
{
...
_Use_decl_annotations_ NTSTATUS IntelVirtDisplayUMDDeviceAdd(WDFDRIVER Driver, PWDFDEVICE_INIT pDeviceInit)
{
	...
	PnpPowerCallbacks.EvtDeviceD0Entry = IntelVirtDisplayUMDDeviceD0Entry;
	PnpPowerCallbacks.EvtDeviceD0Exit = IntelVirtDisplayUMDDeviceD0Exit;
	WdfDeviceInitSetPnpPowerEventCallbacks(pDeviceInit, &PnpPowerCallbacks);

	IDD_CX_CLIENT_CONFIG IntelVirtDisplayConfig;
	IDD_CX_CLIENT_CONFIG_INIT(&IntelVirtDisplayConfig);

	// If the driver wishes to handle custom IoDeviceControl requests, it's necessary to use this callback since IddCx
	// redirects IoDeviceControl requests to an internal queue. This sample does not need this.
	// IntelVirtDisplayConfig.EvtIddCxDeviceIoControl = IntelVirtDisplayUMDIoDeviceControl;

	IntelVirtDisplayConfig.EvtIddCxAdapterInitFinished = IntelVirtDisplayUMDAdapterInitFinished;

	IntelVirtDisplayConfig.EvtIddCxParseMonitorDescription = IntelVirtDisplayUMDParseMonitorDescription;
	IntelVirtDisplayConfig.EvtIddCxMonitorGetDefaultDescriptionModes = IntelVirtDisplayUMDMonitorGetDefaultModes;
	IntelVirtDisplayConfig.EvtIddCxMonitorQueryTargetModes = IntelVirtDisplayUMDMonitorQueryModes;
	IntelVirtDisplayConfig.EvtIddCxAdapterCommitModes = IntelVirtDisplayUMDAdapterCommitModes;
	IntelVirtDisplayConfig.EvtIddCxMonitorAssignSwapChain = IntelVirtDisplayUMDMonitorAssignSwapChain;
	IntelVirtDisplayConfig.EvtIddCxMonitorUnassignSwapChain = IntelVirtDisplayUMDMonitorUnassignSwapChain;

	Status = IddCxDeviceInitConfig(pDeviceInit, &IntelVirtDisplayConfig);
	if (!NT_SUCCESS(Status)) {
		return Status;
	}
	...
	intelvirtdisplay_monitor_count = get_total_screens(g_DevInfo->get_Handle());
	if ((intelvirtdisplay_monitor_count == INTELVIRTDISPLAYUMD_FAILURE) || (intelvirtdisplay_monitor_count > MAX_MONITOR_SUPPORTED)) {
...
_Use_decl_annotations_ NTSTATUS IntelVirtDisplayUMDDeviceD0Entry(WDFDEVICE Device, WDF_POWER_DEVICE_STATE PreviousState)
{
...
NTSTATUS IntelVirtDisplayUMDDeviceD0Exit(WDFDEVICE Device, WDF_POWER_DEVICE_STATE TargetState)
{
...
DeviceInfo::DeviceInfo()
{
	...
	if (get_intelvirtdisplay_kmdf_device() == INTELVIRTDISPLAYUMD_FAILURE) {
		ERR("KMD resource Init Failed\n");
		return;
	}
	// ****** Frame Resources ******
	// Create Device frame Handle to IntelVirtDisplayKMD
 ...
 * Description
 *
 * get_intelvirtdisplay_kmdf_device - This function checks for the IntelVirtDisplayKMD device
 * created and opens a handle for IntelVirtDisplayUMD to use
 *
 * Parameters
 * Null
 *
 ...
 *
 ******************************************************************************/
int DeviceInfo::get_intelvirtdisplay_kmdf_device()
{
	...
	ret = SetupDiEnumDeviceInterfaces(devinfo_handle, 0, &GUID_DEVINTERFACE_INTELVIRTDISPLAYKMD, 0, &device_interface_data);
	if (ret == FALSE) {
	  ...
	  print_counter(0)
{
	...
	// Allocate memory for IOCTL Response Buffer from IntelVirtDisplayKMD (for frame)
	...
	if (hwcursorsupported == TRUE) {
		// ****** Cursor Resources ******
		// Allocate memory for IOCTL Response Buffer from IntelVirtDisplayKMD (for cursor)
		...
		// Create IntelVirtDisplay Cursor Thread
 ...
 * this function will close the IntelVirtDisplay device handle and releases the
	...
	for (;;) {
		...
		if (hr == E_PENDING) {
			...
			if ((g_init_kmd_resources == TRUE)) {
				...
				// Get Frame	from GPU
				if (GetFrameData(m_Device, m_IAcquiredDesktopImage) == INTELVIRTDISPLAYUMD_FAILURE) {
 ...
 * frame from GPU.It passes the frame to IntelVirtDisplayKMD using IOCTL call :
 * set_mode and frame_data
 *
 * Parameters
 * intelvirtdisplay_device - shared_ptr to  Direct3D Device (Direct3D render device)
 ...
 *
 ******************************************************************************/
int SwapChainProcessor::GetFrameData(std::shared_ptr<Direct3DDevice> intelvirtdisplay_device, ID3D11Texture2D *desktopimage)
{
	HRESULT status;
	char err[256];
	memset(err, 0, 256);
	...
	if (m_resolution_changed == TRUE) {
		...
		if (!DeviceIoControl(g_DevInfo->get_Handle(), IOCTL_INTELVIRTDISPLAY_SET_MODE, m_framedata,
...
void SwapChainProcessor::GetCursorData()
{
	...
	UINT tempshapeid = UINT_MAX;
	UINT tempposid = UINT_MAX;
	INT tempX = UINT_MAX;
	INT tempY = UINT_MAX;

	while (1) {
		if (g_init_kmd_resources == TRUE) {
			HANDLE WaitHandles[] = {g_intelvirtdisplay_cursor_os_event[m_screen_num].Get(), m_hTerminateCursorEvent.Get()};
			/* wait for 16ms to get the cursor events from OS */
			WaitResult =
				WaitForMultipleObjects(ARRAYSIZE(WaitHandles), WaitHandles, FALSE, INTELVIRTDISPLAY_CURSOREVENT_WAIT_TIMEOUT);
...
void SwapChainProcessor::ProcessCursorDataLegacy(UINT *tempshapeid, INT *tempX, INT *tempY)
{
	...
	status = IddCxMonitorQueryHardwareCursor(g_IntelVirtDisplayCxMonitorObject[m_screen_num], &g_inputargs[m_screen_num],
											 &m0_outputargs);
	if (!NT_SUCCESS(status)) {
		...
		if (positionChanged) {
			...
			*tempY = m0_outputargs.Y;
			...
			if (!DeviceIoControl(g_DevInfo->get_Handle(), IOCTL_INTELVIRTDISPLAY_CURSOR_POS, m_cursordata,
		...
		if (shapeChanged) {
			*tempshapeid = m0_outputargs.CursorShapeInfo.ShapeId;
			...
			m_cursordata->pitch = m0_outputargs.CursorShapeInfo.Pitch;
			m_cursordata->x_hot = m0_outputargs.CursorShapeInfo.XHot;
			m_cursordata->y_hot = m0_outputargs.CursorShapeInfo.YHot;
			m_cursordata->cursor_x = m0_outputargs.X;
			m_cursordata->cursor_y = m0_outputargs.Y;
			m_cursordata->iscursorvisible = m0_outputargs.IsCursorVisible;
			m_cursordata->data = g_inputargs[m_screen_num].pShapeBuffer;
			m_cursordata->color_format = INTELVIRTDISPLAYUMD_COLORFORMAT;
			if (!DeviceIoControl(g_DevInfo->get_Handle(), IOCTL_INTELVIRTDISPLAY_CURSOR_DATA, m_cursordata,
								 sizeof(struct CursorData), m_ioctlresp_cursor, sizeof(struct KMDF_IOCTL_Response),
								 &m_ioctlresp_size, NULL)) {
				FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(),
							   MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), err, 255, NULL);
...
void SwapChainProcessor::ProcessCursorData(UINT *tempshapeid, UINT *tempposid, INT *tempX, INT *tempY)
{
	...
	status = IddCxMonitorQueryHardwareCursor2(g_IntelVirtDisplayCxMonitorObject[m_screen_num], &g_inputargs[m_screen_num],
											  &m_outputargs);
	if (!NT_SUCCESS(status)) {
		...
		if (shapeChanged) {
			*tempshapeid = m_outputargs.CursorShapeInfo.ShapeId;
			...
			m_cursordata->pitch = m_outputargs.CursorShapeInfo.Pitch;
			m_cursordata->x_hot = m_outputargs.CursorShapeInfo.XHot;
			m_cursordata->y_hot = m_outputargs.CursorShapeInfo.YHot;
			m_cursordata->cursor_x = m_outputargs.X;
			m_cursordata->cursor_y = m_outputargs.Y;
			m_cursordata->iscursorvisible = m_outputargs.IsCursorVisible;
			m_cursordata->data = g_inputargs[m_screen_num].pShapeBuffer;
...
void IndirectDeviceContext::InitAdapter(WDF_POWER_DEVICE_STATE PreviousState)
{
	...
	if (PreviousState != WdfPowerDeviceD3) {
		...
		AdapterCaps.MaxMonitorsSupported = intelvirtdisplay_monitor_count;
		AdapterCaps.EndPointDiagnostics.Size = sizeof(AdapterCaps.EndPointDiagnostics);
		AdapterCaps.EndPointDiagnostics.GammaSupport = IDDCX_FEATURE_IMPLEMENTATION_NONE;
		AdapterCaps.EndPointDiagnostics.TransmissionType = IDDCX_TRANSMISSION_TYPE_WIRED_OTHER;

		// Declare your device strings for telemetry (required)
		AdapterCaps.EndPointDiagnostics.pEndPointFriendlyName = L"IntelVirtDisplayUMD Device";
		AdapterCaps.EndPointDiagnostics.pEndPointManufacturerName = L"Intel";
		AdapterCaps.EndPointDiagnostics.pEndPointModelName = L"IntelVirtDisplayUMD Model";
...
void IndirectDeviceContext::FinishInit(UINT ConnectorIndex)
{
	...
	if (ConnectorIndex >= intelvirtdisplay_monitor_count) {
...
DWORD CALLBACK IndirectDeviceContext::HPDThread(LPVOID Argument)
{
	...
	if (hpd_event_create(AdapterObject) == INTELVIRTDISPLAYUMD_FAILURE) {
...
IndirectMonitorContext::~IndirectMonitorContext()
{
	...
	if (m_cursor_event != INVALID_HANDLE_VALUE) {
		CloseHandle(m_cursor_event);
	}
}

void IndirectMonitorContext::SetupIntelVirtDisplayCursor()
{
	TRACING();
	m_cursor_event = CreateEvent(NULL, false, false, NULL);
	if (m_cursor_event == INVALID_HANDLE_VALUE) {
	...
	g_intelvirtdisplay_cursor_os_event[m_MonitorIndex].Attach(m_cursor_event);
	SetupHwCursor.hNewCursorDataAvailable = g_intelvirtdisplay_cursor_os_event[m_MonitorIndex].Get();
	NTSTATUS Status = IddCxMonitorSetupHardwareCursor(m_Monitor, &SetupHwCursor);

	if (NT_SUCCESS(Status)) {
		g_IntelVirtDisplayCxMonitorObject[m_MonitorIndex] = m_Monitor;
		g_inputargs[m_MonitorIndex].LastShapeId = INITIAL_CURSOR_SHAPE_ID;
		g_inputargs[m_MonitorIndex].ShapeBufferSizeInBytes = CURSOR_BUFFER_SIZE;
		g_inputargs[m_MonitorIndex].pShapeBuffer = (PBYTE)malloc(CURSOR_BUFFER_SIZE);
		if (g_inputargs[m_MonitorIndex].pShapeBuffer == INVALID_HANDLE_VALUE) {
...
void IndirectMonitorContext::AssignSwapChain(IDDCX_SWAPCHAIN SwapChain, LUID RenderAdapter, HANDLE NewFrameEvent)
{
	...
	if (FAILED(Device->Init())) {
		...
		if (hwcursorsupported == TRUE) {
			IndirectMonitorContext::SetupIntelVirtDisplayCursor();
		}
...
_Use_decl_annotations_ NTSTATUS IntelVirtDisplayUMDAdapterInitFinished(IDDCX_ADAPTER AdapterObject,
...
_Use_decl_annotations_ NTSTATUS IntelVirtDisplayUMDAdapterCommitModes(IDDCX_ADAPTER AdapterObject,
...
_Use_decl_annotations_ NTSTATUS IntelVirtDisplayUMDMonitorQueryModes(IDDCX_MONITOR MonitorObject,
...
_Use_decl_annotations_ NTSTATUS IntelVirtDisplayUMDMonitorUnassignSwapChain(IDDCX_MONITOR MonitorObject)
{
	auto *pMonitorContextWrapper = WdfObjectGet_IndirectMonitorContextWrapper(MonitorObject);
...
int hpd_event_create(IDDCX_ADAPTER AdapterObject)
{
	...
	// Create Security Descriptor for HOTPLUG_EVENT and HOTPLUG_TERMINATE_EVENT.
	...
	if (!ConvertStringSecurityDescriptorToSecurityDescriptorW(
		...
		ERR("Failed to create security descriptor for HOTPLUG events, error: %d\n", GetLastError());
		return INTELVIRTDISPLAYUMD_FAILURE;
	}
	...
	if (NULL == hp_terminate_event && GetLastError() == ERROR_ACCESS_DENIED) {
		hp_terminate_event = OpenEvent(EVENT_MODIFY_STATE | SYNCHRONIZE, FALSE, HOTPLUG_TERMINATE_EVENT);
	}
	DWORD hp_term_last_error = GetLastError();
	LocalFree(hp_psd);
	hp_psd = NULL;
	if (NULL == hp_terminate_event) {
		ERR("Cannot create HOTPULG terminate event! GetLastError: %d\n", hp_term_last_error);
		CloseHandle(hp_event);
		return INTELVIRTDISPLAYUMD_FAILURE;
	}
	...
	if (NULL == dve_event && GetLastError() == ERROR_ACCESS_DENIED) {
	...
	dve_psd = NULL;
	if (NULL == dve_event) {
		ERR("Cannot create DVE event! GetLastError: %d\n", dve_last_error);
		CloseHandle(hp_event);
		CloseHandle(hp_terminate_event);
		return INTELVIRTDISPLAYUMD_FAILURE;
	}
	...
	secAttr.lpSecurityDescriptor = shm_psd;
	...
	// Create Security Descriptor for DISP_INFO_MUTEX named mutex.
	// Grants MUTEX_ALL_ACCESS to Local System (SY) and Local Service (LS),
	...
	SECURITY_ATTRIBUTES mtxAttr = {0};
	mtxAttr.nLength = sizeof(mtxAttr);
	mtxAttr.bInheritHandle = FALSE;
	mtxAttr.lpSecurityDescriptor = mtx_psd;

	// Named mutex replaces the former per-process HANDLE stored in shared memory.
	// This handle is valid only in this process; the Enabler opens it by name.
	HANDLE hDispMutex = CreateMutexW(&mtxAttr, FALSE, DISP_INFO_MUTEX);
	LocalFree(mtx_psd);
	mtx_psd = NULL;

	if (hDispMutex == NULL) {
		ERR("Failed to create named mutex for shared memory (%d)\n", GetLastError());
		UnmapViewOfFile(pSharedMem);
		CloseHandle(hSharedMem);
		CloseHandle(hp_event);
		CloseHandle(hp_terminate_event);
	...
	if (get_edid_data(g_DevInfo->get_Handle(), &g_monitors[PRIMARY_IDD_INDEX], PRIMARY_IDD_INDEX,
					  isWin11 ? FALSE : d_edid) == INTELVIRTDISPLAYUMD_FAILURE) {
	...
	while (1) {
		// After user login, first time IntelVirtDisplayEnabler will set this event to enable the HPD Path.
		...
		if (waitstatus == WAIT_OBJECT_0) {
			...
			if (get_hpd_data(devHandle, &hdata) != INTELVIRTDISPLAYUMD_SUCCESS) {
 ...
 * ioctl(IOCTL_INTELVIRTDISPLAY_HP_EVENT) to IntelVirtDisplayKMD to get the current
 * display status which is recieved from QEMU
 *
 * Parameters
 * devHandle - device frame Handle to IntelVirtDisplayKMD
...
int get_hpd_data(HANDLE devHandle, hp_info *data)
{
	...
	if (!DeviceIoControl(devHandle, IOCTL_INTELVIRTDISPLAY_HP_EVENT, g_hdata, sizeof(struct hp_info), g_hdata,
						 sizeof(struct hp_info), &g_bytesReturned, NULL)) {
		FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), err,
					   255, NULL);
		ERR("IOCTL_INTELVIRTDISPLAY_HPD_EVENT call failed with error: %s!\n", err);
		free(g_hdata);
		return INTELVIRTDISPLAYUMD_FAILURE;
	}
...
bool IsWindows11OrLater()
{
	...
	osvi.dwMajorVersion = WINDOWS11_MAJOR_VERSION;
	osvi.dwBuildNumber = WINDOWS11_BUILD_NUMBER;
	...
	DBGPRINT("OS version is later than Windows 11");
	return true;
}
