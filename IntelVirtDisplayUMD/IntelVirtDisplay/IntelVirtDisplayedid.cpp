...
#include <IntelVirtDisplayedid.tmh>
 ...
 * Description
 *
 * get_edid_data - This function gets the handle of IntelVirtDisplayKMD edid device node
 * and send an ioctl(IOCTL_INTELVIRTDISPLAY_GET_EDID_DATA)to IntelVirtDisplayKMD to get edid data
 *
 * Parameters
 * Device frame Handle to IntelVirtDisplayKMD
...
int get_edid_data(HANDLE devHandle, void *m, DWORD id, BOOL d_edid)
{
	...
	static const struct IndirectSampleMonitor s_SampleMonitors[] = {
		// 1080 p EDID
		{{,
	...
	if (!DeviceIoControl(devHandle, IOCTL_INTELVIRTDISPLAY_GET_EDID_DATA, edata, sizeof(struct edid_info), edata,
		...
		FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), err,
					   255, NULL);
		ERR("IOCTL_INTELVIRTDISPLAY_GET_EDID_DATA call failed with error: %s!\n", err);
		free(edata);
		return INTELVIRTDISPLAYUMD_FAILURE;
	}
 ...
 * Description
 *
 * get_total_screens - This function sends an ioctl(IOCTL_INTELVIRTDISPLAY_GET_TOTAL_SCREENS)
 * to IntelVirtDisplayKMD to get the total number of screens
 *
 * Parameters
 * Device frame Handle to IntelVirtDisplayKMD
 *
...
int get_total_screens(HANDLE devHandle)
{
	...
	int ret = INTELVIRTDISPLAYUMD_FAILURE;
	...
	if (!DeviceIoControl(devHandle, IOCTL_INTELVIRTDISPLAY_GET_TOTAL_SCREENS, mdata, sizeof(struct screen_info), mdata,
						 sizeof(struct screen_info), &bytesReturned, NULL)) {
		FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), err,
					   255, NULL);
		ERR("IOCTL_INTELVIRTDISPLAY_GET_TOTAL_SCREENS call failed with error: %s!\n", err);
		free(mdata);
		return ret;
	}
 ...
 * IntelVirtDisplayKMD comparing it from the unsuported reolutions formats which get
 * recieved from Qemu.
 *
 * Parameters
